(function ($) {
    ...
    $(document).ready(function () {
        ...
        function buildChart(idealData, actualData, allIssuesGraph, cleanISS){
            chart = $('#container-burndown').highcharts({
                ...
                tooltip: {
                    ...
                    shared: true,
                    formatter: function() {
                        var result =  '<b>' + Highcharts.dateFormat('%A, %b %e', this.x) + '</b>';
                        $.each(this.points, function(i, datum) {
                            var lastDate = idealData[idealData.length - 1];
                            if(datum.x > (lastDate.date * 1000)) {
                                result += '<br /><span style="color: rgba(255,0,0,0.25)">●</span> Ideal burn: 0<br><span style="color: '+ this.series.color + '">●</span> Actual burn: ' + datum.y;
                            }
                            else {
                                result += '<br /><span style="color:' + this.series.color + '">●</span> ' + this.series.name + ': ' + datum.y;
                            }
                        });
                        return result;
                    }
                ...
                series: [
                {
                    ...
                    lineWidth: 2,
                    dashStyle: "Dash",
                    data: prepareData(idealData, 'ideal')
                },
                {
                    name: 'Actual Burn',
                    type: 'line',
                    ...
                    data: prepareData(cleanISS, 'actual')
                },
                {
                    name: 'All issues',
                    type: 'line',
        ...
        function buildArray(arr){
            ...
            var secondChartArray = [];
            if (typeof arr != 'undefined'){
                for(var i = 0; i < arr.length; i++) {
                    if (currentMilestone.val() == 'all_milestones') {
                        secondChartArray.push(arr[i]);
                    }
                ...
                $.each(data, function(key, item){
                    switch(type){
                        case 'ideal':
                            ...
                            break;
                        case 'actual':
                            tmp.push([moment.utc(item.date * 1000).valueOf(), item.actual]);
                            break;
                        case 'all':
                            tmp.push([moment.utc(item.date * 1000).valueOf(), item.open]);
                    }
                });
        ...
        $('#sel-milestones').change(function (e) {
            ...
            $('.preload2').css({'display':'block', 'z-index':10000, 'background-color': 'rgba(245, 244, 247, 0.4)'});
            ...
            var selectedStyleVal = $('#select-container-list').children('option').filter(':selected').val();
            $.post('index.php?action=select_milestones',{milestone_id:  currentMilestone.val(), selectedStyleValue: selectedStyleVal,project_id: projectID, dateFrom: dateFrom, dateTo: dateTo},  function (resp) {
                if(resp){
                    ...
                    if(selectedStyle.val() == 1) {
                        buildChart(resp.ideal, resp.actual, resp.allIssuesGraph, resp.clean_iss);
